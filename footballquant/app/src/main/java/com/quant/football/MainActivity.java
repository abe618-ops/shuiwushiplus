...
public class MainActivity extends Activity {
    ...
    private void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(12),dp(10),dp(12),dp(8));root.setBackgroundColor(Color.rgb(244,247,248));TextView title=tv("FootballQuant V7.1 · 足球 / 篮球",21,true);title.setTextColor(Color.rgb(28,64,75));root.addView(title);root.addView(tv("SkillFusion · API36 · ZeroGoal · Margin · Spread · Total · ModelDisagreement · 玄学多盘/随机锁盘",11,false));LinearLayout tabs=new LinearLayout(this);tabs.setOrientation(LinearLayout.HORIZONTAL);jc=btn("竞彩");bj=btn("北单");gf=btn("全球足球");bb=btn("篮球");tabs.addView(jc,w());tabs.addView(bj,w());tabs.addView(gf,w());tabs.addView(bb,w());root.addView(tabs);LinearLayout tools=new LinearLayout(this);tools.setOrientation(LinearLayout.HORIZONTAL);Button refresh=btn("刷新当天"),skill=btn("Skill内核"),prev=btn("前一天"),today=btn("今天");tools.addView(refresh,w());tools.addView(skill,w());tools.addView(prev,w());tools.addView(today,w());root.addView(tools);status=tv("准备加载",12,true);status.setPadding(0,dp(6),0,dp(6));root.addView(status);ScrollView sv=new ScrollView(this);content=new LinearLayout(this);content.setOrientation(LinearLayout.VERTICAL);sv.addView(content);root.addView(sv,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);jc.setOnClickListener(v->load("JC"));bj.setOnClickListener(v->load("BJ"));gf.setOnClickListener(v->load("GF"));bb.setOnClickListener(v->load("BB"));refresh.setOnClickListener(v->load(mode));skill.setOnClickListener(v->skillInfo());prev.setOnClickListener(v->{day=shift(day,-1);load(mode);});today.setOnClickListener(v->{day=HistoryStore.today();load(mode);});}
