...
public class HtmlDataSource {
    ...
    public String get(String url,String referer) throws Exception {
        ...
        c.setRequestProperty("User-Agent","Mozilla/5.0 (Linux; Android 16) FootballQuant/7.1.1");
    ...
    private List<MatchInfo> parse500(String h,String src,String day){
        List<MatchInfo> out=new ArrayList<>();
        Pattern row=Pattern.compile("(?is)<tr[^>]*(?:data-fixtureid|data-id|fid)[^>]*>(.*?)</tr>");
        Matcher rm=row.matcher(h);int n=0;
        while(rm.find()&&n<300){
            String r=rm.group(1),t=text(r);String[] teams=teams(r);
            if(invalidTeam(teams[0])||invalidTeam(teams[1]))continue;
            MatchInfo m=new MatchInfo();m.source=src;m.code=find(r,"(?i)(?:data-fixtureid|data-id|fid)=[^0-9]*([0-9]+)");m.matchId=m.code;m.matchDate=day;m.kickoff=find(t,"(\\d{1,2}:\\d{2})");m.league=firstNonEmpty(findByClass(r,"league"),findByClass(r,"league-name"),findText(r,"(?:league|league-name)[^>]*>([^<]+)"));m.home=teams[0];m.away=teams[1];double[] o=odds(t);if(o.length>=3){m.oddsHome=o[0];m.oddsDraw=o[1];m.oddsAway=o[2];}out.add(m);n++;
        }
        return out;
    }

    private void enrichSporttery(List<MatchInfo> ms){try{String raw=get("https://webapi.sporttery.cn/gateway/jc/football/getMatchCalculatorV1.qry?poolCode=had,hhad,crs,ttg,hafu&channel=c","https://www.sporttery.cn/");JSONObject root=new JSONObject(raw);JSONArray list=findArray(root);if(list==null)return;for(int i=0;i<list.length();i++){JSONObject x=list.optJSONObject(i);if(x==null)continue;String home=x.optString("homeTeamName"),away=x.optString("awayTeamName");for(MatchInfo m:ms)if(sim(m.home,home)&&sim(m.away,away)){if(!home.isEmpty())m.home=home;if(!away.isEmpty())m.away=away;JSONArray sp=x.optJSONArray("ttg");if(sp!=null)for(int k=0;k<Math.min(8,sp.length());k++)m.ttgSp[k]=sp.optDouble(k,0);break;}}}catch(Exception ignored){}}
    ...
    private static String[] teams(String r){
        String home=firstNonEmpty(findByClass(r,"home"),findByClass(r,"home-team"),findByClass(r,"team-home"),findByAttr(r,"data-home"),findByAttr(r,"data-homename"));
        String away=firstNonEmpty(findByClass(r,"away"),findByClass(r,"away-team"),findByClass(r,"team-away"),findByAttr(r,"data-away"),findByAttr(r,"data-awayname"));
        if(!invalidTeam(home)&&!invalidTeam(away))return new String[]{home,away};
        List<String>x=new ArrayList<>();Matcher m=Pattern.compile("(?is)<a[^>]*(?:class=[\\\"'][^\\\"']*(?:team|name)[^\\\"']*[\\\"'])?[^>]*>([^<]{2,50})</a>").matcher(r);while(m.find()){String s=text(m.group(1));if(!invalidTeam(s)&&!s.matches(".*(?:析|欧|亚|大|小|荐|直播|数据|赔率).*" )&&!x.contains(s))x.add(s);}if(x.size()>=2)return new String[]{x.get(x.size()-2),x.get(x.size()-1)};
        return new String[]{home,away};
    }
    private static String findByClass(String r,String token){Matcher m=Pattern.compile("(?is)<(?:a|span|div|td)[^>]*class=[\\\"'][^\\\"']*\\b"+Pattern.quote(token)+"\\b[^\\\"']*[\\\"'][^>]*>(.*?)</(?:a|span|div|td)>").matcher(r);return m.find()?text(m.group(1)):"";}
    private static String findByAttr(String r,String attr){Matcher m=Pattern.compile("(?is)"+Pattern.quote(attr)+"\\s*=\\s*[\\\"']([^\\\"']+)[\\\"']").matcher(r);return m.find()?text(m.group(1)):"";}
    private static String firstNonEmpty(String... xs){for(String s:xs)if(s!=null&&!s.trim().isEmpty())return s.trim();return "";}
    private static boolean invalidTeam(String s){return s==null||s.trim().length()<2||s.indexOf('\uFFFD')>=0||s.matches("^[?？�\\-_.· ]+$");}
    ...
    private static String text(String s){return s==null?"":s.replaceAll("(?is)<[^>]+>"," ").replace("&nbsp;"," ").replace("&amp;","&").replace("&quot;","\"").replace("&#39;","'").replaceAll("\\s+"," ").trim();}
